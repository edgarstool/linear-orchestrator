# Tunnel migration: hooks.edgars.tools → Windows localhost:8645
...
- 正式公開 hostname：**hooks.edgars.tools**（複數 webhooks，domain edgars.tools）

Linear webhook 進不來通常是：

1. Dashboard tunnel 裡 **hooks.edgars.tools** 還沒加、或指 **localhost:8644**（舊 hermes gateway）或 **WSL IP:8645**
...
3. 存檔 → 等幾秒 → `https://hooks.edgars.tools/healthz` 應 200
4. Linear App → Webhook URL → `https://hooks.edgars.tools/webhooks/linear`
...
# 公網
Invoke-WebRequest https://hooks.edgars.tools/healthz -UseBasicParsing
```
